  ...
  def this(config: JValue, actionFactory: Option[ActionFactory]) = {
    this(
          ...
          throw new Exception("""JSON configuration for checks are required to have a 'left' or 'field' object"""))),
      {
        ...
        val caseSensitive = JsonUtil.extractOption[Boolean](config \ "caseSensitive")
        operator match {
          ...
          case "true" => new TrueChecker(caseSensitive)
          case "false" => new InverseChecker(new TrueChecker(caseSensitive))
